    ...
    // If it's not, check if check the secrets file
        ...
        if secrets_value.get(name).is_none() {
            logging::error(&format!(
                "Secret {} is not set, please set it with `sm secret add {}`. Alternatively, you can set it in your environment variables.",
                name, name,
            ))
            .await;
            std::process::exit(1);
...
pub async fn get(name: &str) -> String {
    ...
    logging::error(&format!(
            "Secret {} is not set, please set it with `sm secret add {}`. Alternatively, you can set it in your .env file.",
            name, name,
        ))
        .await;
    std::process::exit(1);
...
pub async fn replace_env_vars(text: &str, redact: bool) -> String {
